...
/**
 * The demo clinic's headline numbers, exactly as signup used to copy them into
 * every new clinic. Kept here only to recognise them.
 */
const DEMO_STATS = ['9+|Years of practice', '3,200+|Sessions delivered', '4.9|Average patient rating', '48hr|Typical pain relief']

/**
 * The clinic's own figures — or none, if all it has is the untouched demo set.
 *
 * Clinics created before signup stopped copying them still have "3,200+
 * sessions" and a "4.9" rating stored, which they never typed and cannot all be
 * true. Hiding that exact set here fixes every such clinic without rewriting
 * their data; a clinic that edited even one number keeps what it wrote.
 */
function ownStats(value) {
  const stats = list(value)
  const key = stats.map((s) => `${s?.value}|${s?.label}`).join('\n')
  return key === DEMO_STATS.join('\n') ? [] : stats
}
...
/* --------------------------------------------------------------------------
 *  JOINING FIELDS THAT MAY BE EMPTY
 * --------------------------------------------------------------------------
 *  A new clinic has blanks — no qualifications typed yet, no street address.
 *  Joined blindly, those blanks became visible punctuation on a live website:
 *  "With Dr. Mehta," and ", Pune" and "Physiotherapist ·". These skip the
 *  empty parts, so the sentence reads right however much is filled in.
 * -------------------------------------------------------------------------- */

/** Parts joined with " · ", leaving out the empty ones. */
export function joinDot(...parts) {
  return parts.filter(Boolean).join(' · ')
}

/** "Dr. Mehta, BPT" — or just "Dr. Mehta" before any qualification is entered. */
export function withCredentials(name, credentials) {
  return [name, credentials].filter(Boolean).join(', ')
}

/** The practitioner as one line, e.g. "Dr. Mehta, BPT, MPT (Ortho)". */
export function doctorLine(site) {
  return withCredentials(site?.doctor?.name, site?.doctor?.credentials)
}

/** Street and city, e.g. "12 MG Road, Pune" — or just "Pune". */
export function shortAddress(site) {
  const a = site?.address
