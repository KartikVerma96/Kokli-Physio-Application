...
export async function canSendAuthCodes(clinic) {
  ...
  /**
   * The plan allowing WhatsApp is not the same as WhatsApp working.
   *
   * This used to check only the plan, so with no WHATSAPP_TOKEN on the server a
   * Professional clinic's login page still opened on "Send code on WhatsApp" —
   * and the code went to printToTerminal(), which in production is a log file no
   * patient will ever read. The default way in was a dead end.
   *
   * In development the terminal IS the delivery, so the option stays.
   */
  const deliverable = isWhatsAppConfigured(clinic) || process.env.NODE_ENV !== 'production'
