...
/**
 * How often a signed-in session is checked against the database. See the long
 * note in the jwt callback. Short enough that deactivating somebody takes effect
 * before they finish their next page; long enough that it is not a query per
 * request.
 */
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account, profile }) {
      ...
      if (!slug) {
        // No clinic context — almost always "Continue with Google" on kokli.in by
        // somebody with no account yet. Refusing is the right call: creating a
        // user with no clinic would violate the tenancy CHECK constraint, and
        // silently guessing a clinic would be worse.
        //
        // A URL rather than `false`, because `false` shows up as AccessDenied —
        // "cancelled or not active" — which tells a brand-new visitor nothing.
        // This code gets its own message on the login page saying where to go.
    ...
    async jwt({ token, user, account, trigger }) {
      ...
      /**
       * ---------------------------------------------------------------------
       *  IS THIS ACCOUNT STILL ALLOWED IN?  — re-checked every few minutes
       * ---------------------------------------------------------------------
       *  A JWT is self-contained: once issued it is honoured until it expires,
       *  and this one lives for 30 days. Nothing above re-read the database except
       *  at sign-in and on an explicit client update, so:
       *
       *    * a physiotherapist the owner DEACTIVATED kept reading patient records
       *      for up to a month — "Deactivated. Their records are kept." was true of
       *      the records and false of the access
       *    * a user whose account was deleted kept a working session, which is how
       *      the sign-in button became a loop: the middleware saw a valid token,
       *      bounced them off /login, and the page they landed on had no user
       *    * a suspended clinic's staff stayed signed in, although sign-in itself
       *      already refused them
       *
       *  The note on `updateAge` in auth.config.js suggested it capped this at a
       *  day. It does not: updateAge re-signs the cookie, it never consults the
       *  database.
       *
       *  So every REVALIDATE_SECONDS the row is read again. Missing, inactive or
       *  suspended → `null`, which @auth/core treats as a sign-out and answers by
       *  clearing the session cookie. One indexed primary-key lookup every five
       *  minutes per signed-in user is a cost nobody will ever measure.
       */
      const now = Math.floor(Date.now() / 1000)
      if (user || trigger === 'update') {
        token.checkedAt = now
      } else if (token.id && (!token.checkedAt || now - token.checkedAt > REVALIDATE_SECONDS)) {
        const current = await queryOne(
          `SELECT u.is_active, u.role, u.clinic_id, c.slug AS clinic_slug, c.status AS clinic_status
             FROM users u LEFT JOIN clinics c ON c.id = u.clinic_id
            WHERE u.id = ?`,
          [token.id]
        )
        if (!current || !Number(current.is_active)) return null
        if (current.role !== 'platform' && current.clinic_status === 'suspended') return null

        // A role or clinic changed in the database reaches the session here too,
        // within minutes rather than at the next sign-in.
        token.role = current.role
        token.clinicId = current.clinic_id ? String(current.clinic_id) : null
        token.clinicSlug = current.clinic_slug ?? null
        token.checkedAt = now
      }
